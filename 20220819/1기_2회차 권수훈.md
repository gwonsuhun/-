...
```sql
SELECT is_drinking, blood_pressure, COUNT(*)
FROM healthcare
...
```sql
SELECT height, weight, COUNT(*) AS 사람의수
FROM healthcare
GROUP BY height,weight
ORDER BY 사람의수 DESC LIMIT 5;
...
```sql
SELECT is_drinking AS 음주, smoking AS 흡연, AVG(weight/ ((height/100) * (height/100)) ) AS BMI, COUNT(*)
FROM healthcare
